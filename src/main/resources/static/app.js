...
const Store = {
    ...
    save() {
        ...
        if (currentUser && firebaseReady) saveToFirestore();
    },
...
async function loadFromFirestore() {
    try {
        ...
        if (doc.exists) {
            const data = doc.data();
            if (data.groups && data.groups.length > 0) Store.groups = data.groups;
            if (data.expenses && data.expenses.length > 0) Store.expenses = data.expenses;
            if (data.currency) {
                Store.currency = data.currency;
                document.getElementById('currency-select').value = data.currency;
            }
            // Cache in localStorage
            localStorage.setItem('spliz_groups', JSON.stringify(Store.groups));
            localStorage.setItem('spliz_expenses', JSON.stringify(Store.expenses));
            localStorage.setItem('spliz_currency', Store.currency);
        } else {
            // New user — push localStorage data to Firestore
            saveToFirestore();
        }
    } catch (e) {
